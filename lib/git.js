...
class Git extends EventEmitter {
  ...
  */
  constructor(repoDir, options={}, fsImpl) {
    super();
   ...
   * @memberof Git
   * @param  {String}   dir - directory name
   * @param  {Number}   i - parts index
   * @param  {Function=} callback  - callback to be called when finished
   */
  mkdirR(dir, i, callback) {
    var self = this;
    const parts = this.dirMap(dir).split(path.sep);
    if (i <= parts.length) {
      const directory = parts.slice(0, i).join(path.sep);
      if(directory) {
        this.fs.exists(directory, function (ex) {
          if (!ex) {
            self.fs.mkdir(directory, { recursive: true }, function () {
              self.mkdirR(dir, i + 1, callback);
            });
          } else {
            self.mkdirR(dir, i + 1, callback);
          }
